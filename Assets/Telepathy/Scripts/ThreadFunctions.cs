...
namespace Telepathy
{
    public static class ThreadFunctions
    {
        ...
        public static void ReceiveLoop(int connectionId, TcpClient client, int MaxMessageSize, MagnificentReceivePipe receivePipe, MagnificentReceivePipe receivePipeViewMessages, int QueueLimit, Action onMainReceivePipeCleared)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        // log the reason
                        Log.Warning($"receivePipe reached limit of {QueueLimit} for connectionId {connectionId}. This can happen if network messages come in way faster than we manage to process them. CLEARING PIPE.");

                        // IMPORTANT: do NOT clear the whole queue. we use one
                        // queue for all connections.
                        receivePipe.Clear();

                        if (onMainReceivePipeCleared != null)
                            onMainReceivePipeCleared.Invoke();
                        // just break. the finally{} will close everything.
                        //break;
                    }

                    if (receivePipeViewMessages.Count(connectionId) >= QueueLimit)
                    {
                        // log the reason
                        Log.Warning($"receivePipeViewMessages reached limit of {QueueLimit} for connectionId {connectionId}. This can happen if network messages come in way faster than we manage to process them. CLEARING PIPE.");

                        // IMPORTANT: do NOT clear the whole queue. we use one
                        // queue for all connections.
                        receivePipeViewMessages.Clear();
