...
namespace Telepathy
{
    ...
    class ClientConnectionState : ConnectionState
    {
        ...
        public readonly MagnificentReceivePipe receivePipeViewMessages;
        ...
        public ClientConnectionState(int MaxMessageSize) : base(new TcpClient(), MaxMessageSize)
        {
            ...
            receivePipeViewMessages = new MagnificentReceivePipe(MaxMessageSize);
        }
    ...
    public class Client : Common
    {
        ...
        public Action<ArraySegment<byte>> OnDataVisitorsView;
        ...
        static void ReceiveThreadFunction(ClientConnectionState state, string ip, int port, int MaxMessageSize, bool NoDelay, int SendTimeout, int ReceiveTimeout, int ReceiveQueueLimit, Action onMainReceivePipeCleared)

        {
            ...
            try
            {
                ...
                ThreadFunctions.ReceiveLoop(0, state.client, MaxMessageSize, state.receivePipe, state.receivePipeViewMessages, ReceiveQueueLimit, onMainReceivePipeCleared);
            }
        ...
        public void Connect(string ip, int port, Action onMainReceivePipeCleared)
        {
        ...
        public int TickViewMessages(int processLimit, Func<bool> checkEnabled = null)
        {
            // only if state was created yet (after connect())
            // note: we don't check 'only if connected' because we want to still
            //       process Disconnect messages afterwards too!
            ...
            for (int i = 0; i < processLimit; ++i)
            {
                ...
                if (state.receivePipeViewMessages.TryPeek(out int _, out EventType eventType, out ArraySegment<byte> message))
                {
                    try
                    {
                        OnDataVisitorsView?.Invoke(message);
                    }
                    catch (Exception e)
                    {
                        UnityEngine.Debug.LogException(e);
